...
一个现代化的Go+Vue3全栈项目模板，采用**前后端分离架构**：后端API + 用户端前端。
...
make frontend-build   # 构建前端项目
make frontend-clean   # 清理前端构建文件
make frontend-lint    # 检查前端代码
make frontend-format  # 格式化前端代码
```

### 🚀 全栈命令
```bash
make fullstack-build  # 构建完整全栈应用 (用户端+后端)
