...
# 前端打包脚本
# 用于打包Vue项目并将生成的dist文件夹放置到Go项目的static目录
...
if [ -d "$TARGET_DIST_DIR" ]; then
    ...
    BACKUP_DIR="${STATIC_DIR}/dist_backup_$(date +%Y%m%d_%H%M%S)"
    mv "$TARGET_DIST_DIR" "$BACKUP_DIR"
    echo -e "${GREEN}旧文件已备份到: $BACKUP_DIR${RESET}"
fi

# 复制新的dist目录到static目录
echo -e "${BLUE}复制构建文件到静态目录...${RESET}"
cp -r "$DIST_DIR" "$TARGET_DIST_DIR"

# 显示构建结果
echo -e "${GREEN}前端打包完成!${RESET}"
echo -e "${BLUE}构建信息:${RESET}"
echo -e "  源目录: $DIST_DIR"
echo -e "  目标目录: $TARGET_DIST_DIR"
echo -e "  文件数量: $(find "$TARGET_DIST_DIR" -type f | wc -l)"
echo -e "  总大小: $(du -sh "$TARGET_DIST_DIR" | cut -f1)"

# 显示主要文件
echo -e "${BLUE}主要文件:${RESET}"
ls -la "$TARGET_DIST_DIR"

# 检查关键文件
if [ -f "$TARGET_DIST_DIR/index.html" ]; then
...
# 检查assets目录
if [ -d "$TARGET_DIST_DIR/assets" ]; then
    echo -e "${GREEN}✓ assets目录存在${RESET}"
    echo -e "  CSS文件: $(find "$TARGET_DIST_DIR/assets" -name "*.css" | wc -l)"
    echo -e "  JS文件: $(find "$TARGET_DIST_DIR/assets" -name "*.js" | wc -l)"
else
    echo -e "${YELLOW}⚠ assets目录不存在${RESET}"
fi

echo -e "${GREEN}前端打包流程完成!${RESET}"
echo -e "${BLUE}提示: 现在可以运行 'make build' 来构建包含静态文件的Go二进制文件${RESET}" 