...
import type { RegisterRequest } from '@app/shared'
...
export default function Register() {
  ...
  const canSubmit = form.username.trim().length >= 2 && form.email.trim().length > 0 && form.password.length >= 6 && form.confirmPassword.length > 0 && form.code.trim().length > 0
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setError('')
    if (form.username.trim().length < 2 || form.username.trim().length > 20) { setError('用户名长度为 2-20 个字符'); return }
    if (form.password.length < 6 || form.password.length > 20) { setError('密码长度为 6-20 个字符'); return }
    if (form.password !== form.confirmPassword) { setError('两次输入的密码不一致'); return }
    try {
      const data: RegisterRequest = { username: form.username.trim(), email: form.email.trim(), password: form.password, code: form.code.trim() }
      await register(data)
      success('注册成功，请登录')
      setTimeout(() => navigate('/login'), 800)
    } catch (err: unknown) { setError(err instanceof Error ? err.message : '注册失败，请重试') }
  }
  ...
  return (
    <div className="min-h-screen flex">
      {/* Left dark panel */}
      <div className="hidden lg:flex lg:w-[48%] relative overflow-hidden items-center justify-center" style={{ background: 'linear-gradient(to bottom right, #0f172a, #1e293b)' }}>
        ...
        <div className="relative z-10 px-14 xl:px-20 max-w-lg">
          <div className="flex items-center gap-3 mb-14">
            ...
            <span className="text-white text-xl tracking-tight" style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 700 }}>应用平台</span>
          </div>

          <h1 className="text-4xl font-bold text-white leading-tight tracking-tight">创建账户</h1>
          <p className="mt-4 text-slate-400 text-base leading-relaxed">注册您的账户，即刻体验现代化微前端应用平台的全部功能。</p>

          <div className="mt-14 space-y-5">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0110 0v4" /></svg>
              </div>
              <div>
            ...
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" /></svg>
              </div>
              <div>
            ...
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center shrink-0">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /></svg>
              </div>
              <div>
                <div className="text-sm font-medium text-white">灵活架构</div>
                <div className="text-xs text-slate-500">微前端架构，按需加载子应用</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              ...
              <input id="username" type="text" autoComplete="username" autoFocus required disabled={loading}
                value={form.username} onChange={e => setForm({ ...form, username: e.target.value })}
                placeholder="2-20 个字符" className={inputCls} />
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <input id="password" type={showPwd ? 'text' : 'password'} autoComplete="new-password" required disabled={loading}
            ...
            <div className="mb-6">
              ...
              <input id="confirmPassword" type="password" autoComplete="new-password" required disabled={loading}
                ...
                placeholder="请再次输入密码" className={inputCls} />
            </div>
          ...
          <p className="mt-7 text-center text-sm text-gray-500">
            已有账户？<Link to="/login" className="ml-1 font-semibold text-blue-600 hover:text-blue-700">立即登录</Link>
          </p>
        </div>
      </div>
    </div>
