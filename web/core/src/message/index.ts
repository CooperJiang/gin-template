...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .core-msg-item:hover{transform:scale(1.02)}
    .core-msg-icon{flex-shrink:0;width:24px;height:24px}
    .core-msg-content{flex:1}
    .core-msg-close{flex-shrink:0;padding:4px;opacity:.6;cursor:pointer;border:none;background:none;color:inherit;border-radius:50%;transition:opacity .2s,background .2s}
    .core-msg-close:hover{opacity:1;background:rgba(0,0,0,.1)}
...
function addMessage(options: MessageOptions): string {
  ...
  if (options.showClose) {
    ...
    closeEl.className = 'core-msg-close'
    closeEl.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>`
    closeEl.onclick = () => removeMessage(id)
